...
Phase 1 additions:
  - query-aware visualization filtering
  - executed_query payload for frontend transparency
══════════════════════════════════════════════════════════════════════
"""
...
from app.layers.presentation.viz_reasoner import (
    reason_visualization,
    build_dashboard_layouts,
...
def step_ingest_dataframe(ctx: PipelineContext, df, source_name: str) -> None:
    warnings = validate_dataframe(df, source_name)
    ctx.df = df
    ctx.filename = source_name
    ctx.add_warnings(warnings)
    logger.info(
        "[%s] Ingested dataframe '%s': %d×%d",
        ctx.request_id,
        source_name,
        len(df),
        len(df.columns),
...
def step_query_understand(ctx: PipelineContext) -> None:
    ...
    logger.info(
        ...
        ctx.request_id,
        ctx.intent.question_type,
        ctx.intent.metric,
        ctx.intent.primary_dimension,
    )
...
def step_plan(ctx: PipelineContext) -> None:
    ...
    logger.info(
        "[%s] Plan: %d ops, confidence=%.2f",
        ctx.request_id,
        len(ctx.plan.operations),
        ctx.plan.confidence,
    )
...
def step_execute_and_reflect(ctx: PipelineContext) -> None:
    ...
    logger.info(
        "[%s] Execution: rows=%d valid=%s quality=%.2f",
        ctx.request_id,
        ctx.result.row_count,
        ctx.validation.valid,
        ctx.validation.quality_score,
    )
...
def step_build_vizs(ctx: PipelineContext) -> None:
    ...
    is_overview = str(ctx.intent.question_type) == str(QuestionType.overview)
    ...
    combined = ([ctx.primary_viz] if ctx.primary_viz else []) + ctx.overview_charts

    # Phase 1: make dashboard query-aware
    ctx.all_vizs = filter_vizs_by_intent(combined, ctx.intent)
...
def _serialize_operations(ctx: PipelineContext) -> list[dict[str, Any]]:
    if not ctx.plan:
        return []
...
def _serialize_filters(ctx: PipelineContext) -> list[dict[str, Any]]:
    if not ctx.intent:
        return []
...
def _build_executed_query(ctx: PipelineContext) -> dict[str, Any]:
    return {
        ...
        "filters": _serialize_filters(ctx),
        "formula": ctx.plan.formula_spec if ctx.plan else "",
        "operations": _serialize_operations(ctx),
...
def _build_overview_charts(
    ...
    from app.core.models import QueryIntent, QuestionType
...
def run_pipeline(
    ...
    logger.info(
        "[%s] Pipeline complete in %.2fs — %d vizs, %d warnings",
        ctx.request_id,
        ctx.elapsed,
        len(ctx.all_vizs),
        len(ctx.warnings),
...
def run_pipeline_from_dataframe(
    ...
    """
    Execute the full analysis pipeline from an already loaded dataframe.
    """
    ctx = PipelineContext(prompt=prompt, session_id=session_id)
