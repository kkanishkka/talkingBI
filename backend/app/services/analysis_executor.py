...
══════════════════════════════════════════════════════════════════════
Agent 3: Analysis Executor — fully deterministic, zero LLM.

Fixes:
  - pd.to_datetime uses format="mixed" to silence FutureWarning
  - Row limit: samples >500k DataFrames before execution
  - intermediate_counts: records row count after each step
  - groupby_agg supports multi-column group_by (secondary_dimension)
  - value_counts handles pandas ≥2.0 column rename correctly
══════════════════════════════════════════════════════════════════════
"""
...
def _op_groupby_agg(df: pd.DataFrame, args: dict[str, Any]) -> pd.DataFrame:
    group_by   = args["group_by"]    # list[str]
    target     = args["target"]
    agg_fn     = args["agg_fn"]
    ...
    # convert all group columns to string for JSON safety
    for col in group_by:
...
def _op_value_counts(df: pd.DataFrame, args: dict[str, Any]) -> pd.DataFrame:
    ...
    # pandas ≥2.0: columns are [col, "count"]; <2.0: ["index", col]
...
def execute_plan(
    ...
    """
    Execute an AnalysisPlan deterministically against a DataFrame.
    Never calls an LLM. Returns ExecutionResult.
    """
    ...
    if len(df) > _MAX_ROWS:
        ...
        sample_warning = (
            f"Dataset sampled to {_SAMPLE_ROWS:,} rows from {len(df):,} "
            f"for performance. Results are statistically representative."
        )
        logger.warning("execute_plan: df sampled to %d rows", _SAMPLE_ROWS)

    current_df         = df.copy()
    intermediate_counts: dict[str, int] = {"input": len(current_df)}

    for op_spec in sorted(plan.operations, key=lambda o: o.step):
        op   = op_spec.op
        args = op_spec.args
        handler = _OP_MAP.get(op)
        ...
        try:
            ...
            intermediate_counts[f"after_{op}_step{op_spec.step}"] = len(current_df)
            ...
            return ExecutionResult(
                ...
                error=f"Operation '{op}' (step {op_spec.step}) failed: {exc}",
