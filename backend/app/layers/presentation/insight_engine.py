"""
app/layers/presentation/insight_engine.py
══════════════════════════════════════════════════════════════════════
Dataset-level insight generator — moved from services/insight_engine.py.

Logic is identical. Module path changed to match layered architecture.
Public API: generate_dataset_insights()
══════════════════════════════════════════════════════════════════════
"""
...
def _insight_for_dimension(col: dict[str, Any], total_rows: int) -> dict[str, Any] | None:
    ...
    if null_pct > 10:
        obs.append(f"**{null_pct}%** of records are missing **{name}** — review data quality.")
        priority = "high"
...
def _insight_for_metric(col: dict[str, Any], total_rows: int) -> dict[str, Any] | None:
    ...
    if mn is not None and mx is not None and avg is not None:
        ...
        obs.append(
            f"{prefix}**{name}** ranges {mn:,.2f}–{mx:,.2f}, avg={avg:,.2f}"
            + (f", std={std:,.2f}" if std is not None else "") + "."
        )
        if mx > 0 and (mx - mn) / mx > 0.5:
            obs.append(f"Wide spread ({mx - mn:,.2f}) suggests significant variation or outliers.")
            priority = "high"
    ...
    if null_pct > 5:
        obs.append(f"{null_pct}% of **{name}** values are missing — imputation may be needed.")
        priority = "high"
    ...
    if not obs:
        obs.append(f"**{name}** is a numeric metric with {col['unique_count']:,} unique values.")
...
def _cross_column_insights(schema_profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if high_null:
        insights.append({
            ...
            "insight_text": (
                ...
                f"**{', '.join(high_null)}**. Bias may affect analysis."
            ),
            "category": "data_quality", "priority": "high",
            "evidence_fields": high_null, "confidence": 0.95,
        })
    ...
    if targets:
        ...
        insights.append({
            ...
            "insight_text": (
                ...
                f"Suitable for conversion rate, churn rate, or classification analysis."
            ),
            "category": "structural", "priority": "high",
            "evidence_fields": [t["name"] for t in targets], "confidence": 0.92,
        })
    ...
    if dates:
        insights.append({
            ...
            "category": "trend", "priority": "high",
            "evidence_fields": [d["name"] for d in dates], "confidence": 0.92,
        })

    if total_rows < 100:
        insights.append({
            "title": "Small Dataset",
            "insight_text": f"Only {total_rows} rows — treat results as exploratory, not conclusive.",
            "category": "data_quality", "priority": "medium",
            "evidence_fields": [], "confidence": 0.95,
        })
    elif total_rows > 100_000:
        insights.append({
            "title": "Large Dataset",
            "insight_text": f"{total_rows:,} rows — aggregated views are recommended for readability.",
            "category": "structural", "priority": "low",
            "evidence_fields": [], "confidence": 0.90,
        })
...
def _build_executive_summary(
    ...
    bullets.append(
        f"Dataset: **{total_rows:,} records** × **{total_cols} fields**. Analysed for: {goal}."
    )
    if dimensions:
        bullets.append(
