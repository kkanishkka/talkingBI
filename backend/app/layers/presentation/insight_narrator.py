"""
app/layers/presentation/insight_narrator.py
══════════════════════════════════════════════════════════════════════
Insight Narrator — moved from services/insight_narration_agent.py.

Logic is identical. Module path changed to match layered architecture.
All original LLM + template fallback paths preserved.
══════════════════════════════════════════════════════════════════════
"""
...

{
  ...
  "bullets": [
    "bullet 1 — direct answer with numbers",
    "bullet 2 — comparative insight",
    "bullet 3 — diagnostic pattern",
...
def _build_narrator_message(intent: QueryIntent, result: ExecutionResult) -> str:
    ...
    x, y   = result.x_field, result.y_field
    rows   = result.data[:15]
    lines  = "\n".join(
        f"  {row.get(x, '?')}: {_fmt(row.get(y), metric)}" for row in rows
    )
    return (
...
def _template_narrate(intent: QueryIntent, result: ExecutionResult) -> InsightReport:
    ...
    bullets = [
        ...
        f"**{bot_x}** is the weakest at {bot_y}, compared to the average of {avg_fmt}.",
        f"The spread between top and bottom is **{ratio_str}** — worth investigating.",
    ]
...
def build_assumption_block(intent: QueryIntent, plan_formula: str) -> AssumptionBlock:
    metric_assumption = ""
